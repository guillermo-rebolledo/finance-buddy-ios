...
- **Core / Stores:** Main-actor `@Observable` feature stores own state and call the protocol. Period request sequencing prevents stale replies. The Dashboard commits summary and trends together after checking their period identity. A failed refresh preserves the loaded snapshot. `BudgetsStore` lists budgets and stops or removes them; `BudgetEditorStore` resolves every chosen period through the summary endpoint before it names, prefills, or saves a budget.
...
The Budgets tab sits between Dashboard and Categories. It lists Now (today, this week, and this month, each with its budget or a Set budget action), Repeating (each span with Change and Stop, the latter behind a confirmation), Upcoming one-offs (Change and Remove), and Past (read-only, twenty at a time with Show more). The form takes a period kind, a date, an amount that may be zero, and a One-off toggle; the server resolves the period, so the form shows its label and prefills the budget that already applies. An ended period is announced and cannot be saved, and a `period_ended` refusal is shown as-is. Budget writes are idempotent, so a retry after a lost reply repeats the same request without a kept identity. The dashboard shows the selected period's budget read-only with a link to the tab, and the toast after saving an expense or refund appends the budget line the reply carries, such as "Entry saved. MXN 427.50 left this week." A summary without a `budget` field, from an older backend, still decodes.
...
Use `-useFakeAPI` in a Debug scheme’s launch arguments to explore without signing in. Optional fixture-only arguments `-light`, `-dark`, and `-largestType` support visual verification. The live client ignores them. For fixture-only screen inspection, add `-previewScreen` followed by `editor`, `categoryEditor`, `budgetEditor`, `budgets`, `deletion`, `export`, `signIn`, `unavailable`, or `upgrade`; this route is available only with `-useFakeAPI` in Debug. `ScreenPreviews.swift` provides light, dark, and largest-size previews for every app screen, plus empty-day and failed-load scenarios. The normal fixture includes income, expenses, a refund, an archived category, a twelve-week trend, a repeating weekly budget, and an upcoming one-off week. The in-memory client keeps budget spans the way the server does, so previews and UI tests exercise changes, one-offs, stops, and Past paging without a backend.
