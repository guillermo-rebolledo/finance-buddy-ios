  ...
  var body: some View {
    ScrollView {
      VStack(spacing: 28) {
        ...
        VStack(spacing: 12) {
          Text("Finance Buddy").font(.largeTitle.bold())
        ...
        Button {
          ...
          .frame(minHeight: 44).disabled(signingIn || session.busy || session.client.access.offline)
        ...
        Text("Access is limited to the workspace owner.").font(.footnote).foregroundStyle(
          .secondary)
      }.multilineTextAlignment(.center).padding(32).containerRelativeFrame(.vertical) { length, _ in
        max(460, length)
      }
    }
  }
